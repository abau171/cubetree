...
usage: python3 run.py [-g|--gen-lookups]
       python3 run.py [-s|--server] [-h|--hostname=<hostname>] \
[-p|--port=<port>]
       python3 run.py [-w|--workers=<num_workers>] [-h|--hostname=<hostname>] \
[-p|--port=<port>]
...
def main():
    ...
    try:
        ...
        opts, args = getopt.getopt(sys.argv[1:], "gsw:h:p:P",
            ["server", "workers=", "hostname=", "port=", "profile"])
